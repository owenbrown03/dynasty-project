...
@pytest.mark.anyio
async def test_get_commissioner_waivers_overview_preserves_sleeper_order(monkeypatch):
    mock_db = AsyncMock()
    ctx = SimpleNamespace(
        db=mock_db,
        redis=None,
    ...
    # League Z is first in Sleeper, League A is second
    league_z = SimpleNamespace(
        league_id="lz",
        name="Zebra League",
        avatar=None,
        total_rosters=12,
        settings={"daily_waivers": 1, "daily_waivers_days": 5461},
    )
    ...
    monkeypatch.setattr(
        "app.services.commissioner.waivers.get_league_sort_orders",
