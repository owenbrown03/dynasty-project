...
async def sync_leagues(db: AsyncSession, raw_leagues: list[dict], curr_week: int, sleeper: SleeperClient) -> dict:
    if curr_week < 1:
        curr_week = 1
    ...
    try:
        ...
        async def fetch_with_progress(league_data):
            ...
            try:
                return await fetch_league_bundle(league_data, curr_week, sleeper)
            finally:
                processed_fetches += 1
...
async def fetch_league_bundle(league_dict: dict, curr_week: int, sleeper: SleeperClient) -> Optional[dict]:
    ...
    try:
        ...
        trade_tasks = [sleeper.read.get_transactions(league_id, week) for week in range(1, curr_week + 1)]
